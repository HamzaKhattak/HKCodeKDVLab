...
void loop() {
  //Handle getting strings from serial
  getString();
  //If a string is received do processing on the string
  if (newdata == true) {
...
void performFunction() {
  ...
  //Turn motors on and off for manual movement
      if (strncmp("EN",messageFromPC,2) == 0) 
  {
     Serial.println("Enabling motors");
    digitalWrite(rotdisablepin,LOW);
    digitalWrite(movedisablepin,LOW);
  }
        if (strncmp("EF",messageFromPC,2) == 0) 
  {
     Serial.println("Disabling motors");
    digitalWrite(rotdisablepin,HIGH);
    digitalWrite(movedisablepin,HIGH);
  }
