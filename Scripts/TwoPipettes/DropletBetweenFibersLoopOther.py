...
#for i in [10,11,12]:
	...
	allimages = pims.PyAVReaderIndexed(run_names[i])
	allimages = np.array(allimages,dtype=np.uint8)
	allimages = allimages[:,y0:y1,x0:x1,0]
	leadtxt = run_names[i].split('.')[0]
	'''
	allimages= ndt.NDTiffDataset(run_names[i][:-1]).as_array()[0,:,0,0]
	allimages = np.array(allimages,dtype = np.uint8)
	allimages = allimages[:,y0:y1,x0:x1]
	leadtxt = run_names[i][:-1]
	'''
